#!/usr/bin/env python3
# journal_daemon_v6_secure.py — V6.0.0
...
def main(argv: list[str] | None = None) -> int:
    ...
    while True:
        ...
        params_strategie: dict[str, Any] = {}
        if isinstance(config, dict):
            params_strategie.update(config)
        mode_execution = str(params_strategie.get("mode_execution") or "simulation")
        params_strategie["mode_execution"] = mode_execution
        params_strategie["mode_global"] = etat.get("mode_global")
        ...
        try:
            plan_reeq = generer_plan_reequilibrage_contexte(
                ...
                signaux_normalises=signaux_norm,
                params_strategie=params_strategie,
                run_id=run_id,
                journal_path=str(STRATEGY_JOURNAL_PATH),
            )
        except Exception as exc:
            print(f"[ERROR] Echec critique de génération du plan de rééquilibrage : {exc}")
            break
