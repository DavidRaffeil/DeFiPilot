# core/swap_reel_v3_7_candidate_liquidity_fix_v3.py - V3.7-candidate-liquidity-fix-v3
"""
...
def _get_min_amount_out_human(token_out: str) -> float:
    token_out_lower = token_out.lower()
    if token_out_lower in {USDC_POLYGON.lower(), USDC_E_POLYGON.lower()}:
        return 0.01
    if token_out_lower == WETH_POLYGON.lower():
        return 0.000001
    return 0.000001
...
def effectuer_swap_reel(
    ...
    *,
    ...
    token_out_contract = _erc20(w3, token_out)
    decimals_out = int(token_out_contract.functions.decimals().call())
    amount_out_human = amounts[-1] / (10 ** decimals_out)
    min_amount_out_human = _get_min_amount_out_human(token_out)
    if amount_out_human < min_amount_out_human:
        logger.warning("Swap bloqué : liquidité de pool insuffisante")
        return {"status": "BLOCKED", "reason": "liquidite_pool_insuffisante"}
