# liquidity_real_tx_v6_candidate_v3_confirm_fix_abi_fix_v2.py – DeFiPilot V6.0
...
# Dossier ABI résolu en priorité vers core/abis depuis core/execution/
...
def _wait_receipt_robust(w3: Web3, tx_hash, timeout: int = 180, poll_interval: float = 2.0):
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if tx is None:
                last_error = RuntimeError("transaction introuvable")
        except Exception as exc:
            last_error = exc
    ...
    try:
        return w3.eth.wait_for_transaction_receipt(tx_hash, timeout=max(1, timeout // 3))
    except Exception as exc:
        if last_error is not None:
            raise RuntimeError(f"confirmation timeout/erreur: {last_error}") from exc
        raise
                    ...
                    try:
                        ...
                        if factory_addr:
                            pair_addr = (
                                ...
                                .functions.getPair(tokenA_cs, tokenB_cs)
                                .call()
                            )
