# liquidity_real_tx_v6_candidate_v3_confirm_fix_abi_fix_v4.py – DeFiPilot V6.0
...
- Confirmation robuste conservée.
- Chemin ABI conservé vers core/abis.
- Journalisation corrigée : appels avec un seul dict.
"""
...
# Dossier ABI relatif à core/execution/ -> core/abis
...
def _wait_receipt_robust(w3: Web3, tx_hash, timeout: int = 180, poll_latency: float = 1.5):
    txh = tx_hash.hex() if hasattr(tx_hash, "hex") else str(tx_hash)
    start = time.time()
    while time.time() - start < timeout:
        try:
            receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=30, poll_latency=poll_latency)
    ...
    raise TimeoutError(f"Confirmation timeout pour tx {txh} après {timeout}s")
...
def ajouter_liquidite_reelle(
    *,
    ...
    # =====================
    # Journalisation standard (schéma V3.8 conservé)
    # =====================
    try:
