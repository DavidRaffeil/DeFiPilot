# liquidity_real_tx_v6_candidate_v3_confirm_fix.py – DeFiPilot V6.0
"""Transactions d'ajout de liquidité (réel) + journalisation CSV/JSONL.

Correctif ciblé V6.0:
- Confirmation on-chain plus robuste pour éviter les faux négatifs après timeout initial.
- Aucun changement de logique métier (montants, approve, slippage, pair-check, signature, envoi, lecture LP).
"""
...
from web3.exceptions import ABIFunctionNotFound, ContractLogicError, TimeExhausted, TransactionNotFound
...
# Paramètres de confirmation robustes (corrigent uniquement les faux timeouts)
INITIAL_CONFIRM_TIMEOUT_S = 120
EXTRA_CONFIRM_ATTEMPTS = 4
EXTRA_CONFIRM_SLEEP_S = 15
...
def _wait_receipt_robust(w3: Web3, tx_hash: bytes):
    """Attend la confirmation avec fallback prudent pour éviter les faux négatifs."""
    tx_hash_hex = tx_hash.hex()

    try:
        logger.info(
            "[V6.0] ⏳ Attente normale de confirmation tx=%s (timeout initial=%ss)",
            tx_hash_hex,
            INITIAL_CONFIRM_TIMEOUT_S,
        )
        return w3.eth.wait_for_transaction_receipt(tx_hash, timeout=INITIAL_CONFIRM_TIMEOUT_S)
    except TimeExhausted:
        logger.warning(
            "[V6.0] ⚠️ Délai initial dépassé pour tx=%s. Vérification on-chain supplémentaire en cours...",
            tx_hash_hex,
        )

    # Relecture prudente de la receipt (évite de conclure trop vite à un échec)
    for attempt in range(1, EXTRA_CONFIRM_ATTEMPTS + 1):
        try:
            receipt = w3.eth.get_transaction_receipt(tx_hash)
            if receipt is not None and getattr(receipt, "blockNumber", None) is not None:
                logger.info(
                    "[V6.0] ✅ Transaction finalement confirmée après délai tx=%s block=%s status=%s",
                    tx_hash_hex,
            ...
            logger.info(
                "[V6.0] 🔎 Vérification supplémentaire %s/%s: receipt introuvable pour tx=%s",
                attempt,
                EXTRA_CONFIRM_ATTEMPTS,
                tx_hash_hex,
            )
        except Exception as exc:
            logger.warning(
                "[V6.0] ⚠️ Vérification supplémentaire %s/%s en erreur pour tx=%s: %s",
                attempt,
                EXTRA_CONFIRM_ATTEMPTS,
                tx_hash_hex,
                exc,
        ...
        if attempt < EXTRA_CONFIRM_ATTEMPTS:
            time.sleep(EXTRA_CONFIRM_SLEEP_S)

    # Dernière tentative via wait court
    try:
        logger.info("[V6.0] 🔁 Dernière attente courte de confirmation tx=%s", tx_hash_hex)
        receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=EXTRA_CONFIRM_SLEEP_S)
        if receipt is not None:
            logger.info(
                "[V6.0] ✅ Transaction confirmée lors de la dernière vérification tx=%s block=%s status=%s",
                tx_hash_hex,
                getattr(receipt, "blockNumber", None),
                getattr(receipt, "status", None),
            )
    ...
    logger.error(
        "[V6.0] ❌ Échec final confirmation tx=%s: introuvable après vérifications supplémentaires",
        tx_hash_hex,
    )
    return None
...
def ajouter_liquidite_reelle(
    *,
    ...
    if wallet is None:
        ...
        logger.error("[V6.0] ⚠️ %s", error_msg)
    else:
        try:
            rpc_url = None
            ...
            if dry_run:
                ...
                logger.info(
                    "[V6.0] DRY-RUN addLiquidity %s/%s A=%s B=%s slippage=%s",
                    tokenA_symbol,
                    tokenB_symbol,
                    amountA,
                    amountB,
                ...
                logger.info("[V6.0] ✅ Tx envoyée: %s", tx_hash_str)

                receipt = _wait_receipt_robust(w3, tx_hash)
                if receipt is None:
                    success = False
                    tx_status = "failed"
                    error_msg = "Transaction non retrouvée après délai initial et vérifications supplémentaires"
                else:
                ...
                if receipt is not None and success:
    ...
    try:
        ...
        logger.error("[V6.0] ⚠️ Journalisation standard échouée: %s", log_exc)

    logger.info(
        "[V6.0] Résumé: platform=%s, pair=%s-%s, amountA=%s, amountB=%s, slippage=%s, dry_run=%s, tx=%s, lp=%s",
        platform,
        tokenA_symbol,
        tokenB_symbol,
        amountA,
